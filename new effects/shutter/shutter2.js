function vrbShutter2(selector, pieces =4, duration =5){
    ...
    lis.eq(0).show();
    var unitHeight=shutter.innerHeight()/pieces;
    ...
    function rotateRight(){
        ...
        for(var n=0;n<pieces;n++){
            ...
            var front=$("<div>").addClass("front").css({
                ...
                "background-position":`0 -${unitHeight*n}px`
            }).appendTo(group);
            ...
            front.add(left).css({
                "transform-origin":`50% 50% -${unitHeight/2}px`
            });
        }
        n=0;
        var timer=setInterval(function(){
            var group=shutter.find(".group").eq(n++);
            group.find(".left").css("transform","rotateX(0)");
            group.find(".front").css("transform","rotateX(-90deg)");
            if(n==pieces) {
                clearInterval(timer);
            }
        },100);
